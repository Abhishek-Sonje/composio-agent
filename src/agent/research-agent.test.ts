...
describe("V2 gated buildability derivation", () => {
  it("derives partially buildable and preserves an enterprise access requirement", () => {
    const normalized = removeUnsupportedClaims({
      ...result,
      authMethods: ["OAuth 2.0"],
      accessModel: "enterprise_only",
      apiSurface: { ...result.apiSurface, rest: true },
      buildability: "unknown",
      blocker: null,
      evidence: [
    ...
    expect(normalized.buildability).toBe("partially_buildable");
    expect(normalized.blocker).toMatch(/enterprise/i);
    expect(normalized.unknownFields).not.toContain("buildability");
    expect(normalized.unknownFields).not.toContain("blocker");
  });

  it("does not derive blocked merely because access is approval-gated", () => {
    const normalized = removeUnsupportedClaims({
      ...result,
      authMethods: ["OAuth 2.0"],
      accessModel: "partnership_required",
      apiSurface: { ...result.apiSurface, rest: true },
      buildability: "unknown",
      blocker: null,
      evidence: [
    ...
    expect(normalized.buildability).toBe("partially_buildable");
    expect(normalized.buildability).not.toBe("blocked");
  });
});
